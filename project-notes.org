...
- create suds with https://docs.rs/opml/1.1.6/opml/struct.Outline.html#method.add_feed
...
mod test {
    ...
    // OPML tests
    // import -> from example.opml to Vec<Feed> -> match content
    // export -> from OPML Example to example.opml -> match content
    // delete -> from OPML Example file -> check emtpy? deleted?
